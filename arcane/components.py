    ...
    def step(self):
        ...
        if e_before > 0:
            ...
            for output_comp in range(self.n_outputs):
                nxt = self.next_comp[output_comp]
                if nxt.allow_input and not nxt.self_managed_links:
                    allowed_outputs.append(output_comp)
            n_allowed_out = len(allowed_outputs)
            remaining_idx = []
            for i in allowed_outputs:
                if self.next_comp[i].energy + e_before / \
                        n_allowed_out < self.next_comp[i].max_energy:
